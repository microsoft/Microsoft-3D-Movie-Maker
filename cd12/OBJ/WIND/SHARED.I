...
CHUNK( 'GLOP', 0x00050001,  ) SCRIPT
	PrintStr("Running shared init script");
...
ENDCHUNK
	...
	SET _dfscr++ CHUNK( 'GLOP', _dfscr,  ) PARENT( 'GOKD', _curgok, 0x0021 ) SCRIPT
		If (!(_parm[0] != 0)); AlertStr("Assertion failed: ", "H:\\BUILD\\SOC\\src\\shared\\util.cht", ", line ", NumToStr(161,"")); End;
		If (!(_parm[1] >= 0x0001)); AlertStr("Assertion failed: ", "H:\\BUILD\\SOC\\src\\shared\\util.cht", ", line ", NumToStr(162,"")); End;
		PrintStr("Moving to ", NumToStr(_parm[0], ""), " with state = ", NumToStr(_parm[1], ""));
	...
	SET _dfscr++ CHUNK( 'GLOP', _dfscr,  ) PARENT( 'GOKD', _curgok, 0x0026 ) SCRIPT
		...
		Elif (_parm[1] == 40042 && (((0x00010000->keys) & 0x00000001) == 0x00000001));
			If (::fInStudio);
				PrintStr("Bringing up map from studio");
				CreateChildGob(0x20002, 0x00012000, 0x00012000);
			Else;
				PrintStr("Bringing up map from building");
	...
	SET _dfscr++ CHUNK( 'GLOP', _dfscr,  ) PARENT( 'GOKD', _curgok, 0x0028 ) SCRIPT
		If (!(_parm[0] != 0)); AlertStr("Assertion failed: ", "H:\\BUILD\\SOC\\src\\shared\\util.cht", ", line ", NumToStr(331,"")); End;
		PrintStr("Util: Loading Studio");
		StartLongOp();
	...
	SET _dfscr++ CHUNK( 'GLOP', _dfscr,  ) PARENT( 'GOKD', _curgok, 0x0029 ) SCRIPT
		PrintStr("Load Studio Failed handler");
		::fInStudio = 0;
...
SET _dfscr++ CHUNK( 'GLOP', _dfscr,  ) PARENT( 'GOKD', _curgok, 0x0021 ) SCRIPT
	...
	If ( ::fInStudio );
		If( fTools );
			...
			PrintStr("Studio=>Studio via Map");
		...
		PrintStr("Studio=>Building via Map");
	...
	Else;
		If (GidParThis() == newplace);
			...
			PrintStr("Building=>Same place in Building via Map");
			If (0x00010000->cDisable <= 0 && !0); PrintStr("Enable Accels takes disable count below 0."); End; If (0); If (0x00010000->cDisable >= 1); EnqueueCid(50094, 0, 0, 0, 0, 0); End; 0x00010000->cDisable = 0; Else; 0x00010000->cDisable--; If (0x00010000->cDisable == 0); EnqueueCid(50094, 0, 0, 0, 0, 0); End; End;
			DestroyThis();		
			...
			If (newstate != Match( newplace, 0x0001, 0x00011c00, 0x0001, 0x00011000, 0x0002, 0x00010c00, 0x0004, 0x00010600, 0x0002, 0x00010580, 0x0003, 0x00011400, 0x0006, 0x00011800, 0x0001));
				PrintStr("Changing state to ", NumToStr(newstate, ""));
		...
		PrintStr("Building=>Building or Studio via Map");
		If (0x00010000->cDisable <= 0 && !0); PrintStr("Enable Accels takes disable count below 0."); End; If (0); If (0x00010000->cDisable >= 1); EnqueueCid(50094, 0, 0, 0, 0, 0); End; 0x00010000->cDisable = 0; Else; 0x00010000->cDisable--; If (0x00010000->cDisable == 0); EnqueueCid(50094, 0, 0, 0, 0, 0); End; End;
		DestroyThis();		
...
SET _dfanm++ CHUNK( 'GLSC', _dfanm,  ) PARENT( 'GOKD', _curgok, SET _st = 0x10000 SET _st |= 0x0010 _st ) SCRIPT
	...
	If (!(x != -1 && y != -1)); AlertStr("Assertion failed: ", "H:\\BUILD\\SOC\\src\\shared\\map.cht", ", line ", NumToStr(263,"")); End;
	MoveAbsThis( x, y );
...
ENDCHUNK
SET _dfscr++ CHUNK( 'GLOP', _dfscr,  ) PARENT( 'GOKD', _curgok, 0x0021 ) SCRIPT
	If (!(::nBioPage >= 0)); AlertStr("Assertion failed: ", "H:\\BUILD\\SOC\\src\\shared\\biopage.cht", ", line ", NumToStr(37,"")); End;	
	PrintStr("Updating bio pages object");

	If( ::nBioPage < 0 || ::nBioPage > 41 );
		AlertStr( "Invalid bio page start: default to 0 [toc]" );
		::nBioPage = 0;
	End;
